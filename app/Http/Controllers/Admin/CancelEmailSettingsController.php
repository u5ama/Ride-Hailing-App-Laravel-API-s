...
class CancelEmailSettingsController extends Controller
{
    /**
     * Display a listing of the CancelEmailSettings.
     *
    ...
    public function index(Request $request)
    {
        ...
        $headerTrans = EmailHeader::listsTranslations('name')->select('email_headers.id', 'email_headers.emh_logo', 'email_header_translations.emh_subject', 'email_header_translations.emh_title', 'email_header_translations.emh_description')->where('email_headers.id', 3)->first();
        $bodyTrans = EmailBody::listsTranslations('name')->select('email_bodies.id', 'email_body_translations.emb_title_text_bf_name', 'email_body_translations.emb_title_text_after_name', 'email_body_translations.emb_body_text_bf_button', 'email_body_translations.emb_body_text_after_button')->where('email_bodies.id', 3)->first();
        $footerTrans = EmailFooter::listsTranslations('name')->select('email_footers.id', 'email_footer_translations.emf_company_name', 'email_footer_translations.emf_company_address', 'email_footer_translations.emf_company_contacts')->where('email_footers.id', 3)->first();

        return view('admin.CancelEmailSettings.index', ['languages' => $languages, 'header' => $header, 'headerTrans' => $headerTrans, 'bodyTrans' => $bodyTrans, 'footerTrans' => $footerTrans]);
    }

    /**
     * Show the form for creating a new CancelEmailSettings.
     *
     * @return Factory|View
     */
    ...
    {
        $languages = Language::where('status', 1)->get();
        return view('admin.CancelEmailSettings.create', ['languages' => $languages]);
    }

    /**
     * Store a newly created FormOne for CancelEmailSettings in storage.
     *
     * @param Request $request
     * @return JsonResponse
     */
    public function FormOneCreate(Request $request)
    {
        if ($request->hasFile('emh_logo')) {
            ...
            $logo->move('./assets/CancelEmail/logo/', $logoName);
            $welcomeEmailLogo = 'assets/CancelEmail/logo/' . $logoName;
        }else{
            $welcome = EmailHeader::where('id',3)->first();
    ...
    /**
     * Store a newly created FormTwo for CancelEmailSettings in storage.
     *
     * @param Request $request
     * @return JsonResponse
     */
    ...
    /**
     * Store a newly created FormThree for resource in storage.
     *
     * @param Request $request
     * @return JsonResponse
     */
    ...
    /**
     * Display the specified CancelEmailSettings.
     *
     * @param int $id
     * @return Factory|View
     */
    public function show($id)
    {
        ...
        return view('admin.CancelEmailSettings.show', ['languages' => $languages, 'header' => $header, 'headerTrans' => $headerTrans,
