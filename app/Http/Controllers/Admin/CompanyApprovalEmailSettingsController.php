...
class CompanyApprovalEmailSettingsController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        $header = EmailHeader::where('id', 6)->first();
        $headerTrans = EmailHeader::listsTranslations('name')->select('email_headers.id', 'email_headers.emh_logo', 'email_header_translations.emh_subject', 'email_header_translations.emh_title', 'email_header_translations.emh_description')->where('email_headers.id', 6)->first();
        $bodyTrans = EmailBody::listsTranslations('name')->select('email_bodies.id', 'email_body_translations.emb_title_text_bf_name', 'email_body_translations.emb_title_text_after_name', 'email_body_translations.emb_body_text_bf_button', 'email_body_translations.emb_body_text_after_button')->where('email_bodies.id', 6)->first();
        $footerTrans = EmailFooter::listsTranslations('name')->select('email_footers.id', 'email_footer_translations.emf_company_name', 'email_footer_translations.emf_company_address', 'email_footer_translations.emf_company_contacts')->where('email_footers.id', 6)->first();
        return view('admin.CompanyApprovalEmailSettings.index', ['languages' => $languages, 'header' => $header, 'headerTrans' => $headerTrans, 'bodyTrans' => $bodyTrans, 'footerTrans' => $footerTrans]);
    }
    ...
    {
        $languages = Language::where('status', 1)->get();
        return view('admin.CompanyApprovalEmailSettings.create', ['languages' => $languages]);
    }
    ...
    public function FormOneCreate(Request $request)
    {
        if ($request->hasFile('emh_logo')) {
            ...
            $logo->move('./assets/CompanyApprovalEmail/logo/', $logoName);
            $welcomeEmailLogo = 'assets/CompanyApprovalEmail/logo/' . $logoName;
        }else{
            $welcome = EmailHeader::where('id',6)->first();
    ...
    public function show($id)
    {
        ...
        return view('admin.welcomeCompanyEmailSettings.show', ['languages' => $languages, 'header' => $header, 'headerTrans' => $headerTrans,
