...
class DriverApprovalEmailSettingsController extends Controller
{
    /**
     * Display a listing of the DriverApprovalEmailSettings.
     *
    ...
    public function index(Request $request)
    {
        ...
        $headerTrans = EmailHeader::listsTranslations('name')->select('email_headers.id', 'email_headers.emh_logo', 'email_header_translations.emh_subject', 'email_header_translations.emh_title', 'email_header_translations.emh_description')->where('email_headers.id', 8)->first();
        $bodyTrans = EmailBody::listsTranslations('name')->select('email_bodies.id', 'email_body_translations.emb_title_text_bf_name', 'email_body_translations.emb_title_text_after_name', 'email_body_translations.emb_body_text_bf_button', 'email_body_translations.emb_body_text_after_button')->where('email_bodies.id', 8)->first();
        $footerTrans = EmailFooter::listsTranslations('name')->select('email_footers.id', 'email_footer_translations.emf_company_name', 'email_footer_translations.emf_company_address', 'email_footer_translations.emf_company_contacts')->where('email_footers.id', 8)->first();
        return view('admin.DriverApprovalEmailSettings.index', ['languages' => $languages, 'header' => $header, 'headerTrans' => $headerTrans, 'bodyTrans' => $bodyTrans, 'footerTrans' => $footerTrans]);
    }

    /**
     * Show the form for creating a new DriverApprovalEmailSettings.
     *
     * @return Factory|View
     */
    ...
    {
        $languages = Language::where('status', 1)->get();
        return view('admin.DriverApprovalEmailSettings.create', ['languages' => $languages]);
    }

    /**
     * Store a newly created DriverApprovalEmailSettings in storage.
     *
     * @param Request $request
     * @return JsonResponse
     */
    public function FormOneCreate(Request $request)
    {
        if ($request->hasFile('emh_logo')) {
            ...
            $logo->move('./assets/DriverApprovalEmail/logo/', $logoName);
            $welcomeEmailLogo = 'assets/DriverApprovalEmail/logo/' . $logoName;
        }else{
            $welcome = EmailHeader::where('id',8)->first();
    ...
    /**
     * Display the specified DriverApprovalEmailSettings.
     *
     * @param int $id
     * @return Factory|View
     */
    public function show($id)
    {
        ...
        return view('admin.DriverApprovalEmailSettings.show', ['languages' => $languages, 'header' => $header, 'headerTrans' => $headerTrans,
            'bodyTrans' => $bodyTrans, 'footerTrans' => $footerTrans, 'socialLinks' => $socialLinks]);
    }

    /**
     * Show the form for editing the specified DriverApprovalEmailSettings.
     *
     * @param int $id
     * @return void
     */
