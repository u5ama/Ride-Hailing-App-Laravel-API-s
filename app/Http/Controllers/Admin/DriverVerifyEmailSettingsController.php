...
class DriverVerifyEmailSettingsController extends Controller
{
    /**
     * Display a listing of the DriverVerifyEmailSettings.
     *
    ...
    public function index(Request $request)
    {
        ...
        $headerTrans = EmailHeader::listsTranslations('name')->select('email_headers.id', 'email_headers.emh_logo', 'email_header_translations.emh_subject', 'email_header_translations.emh_title', 'email_header_translations.emh_description')->where('email_headers.id', 9)->first();
        $bodyTrans = EmailBody::listsTranslations('name')->select('email_bodies.id', 'email_body_translations.emb_title_text_bf_name', 'email_body_translations.emb_title_text_after_name', 'email_body_translations.emb_body_text_bf_button', 'email_body_translations.emb_body_text_after_button')->where('email_bodies.id', 9)->first();
        $footerTrans = EmailFooter::listsTranslations('name')->select('email_footers.id', 'email_footer_translations.emf_company_name', 'email_footer_translations.emf_company_address', 'email_footer_translations.emf_company_contacts')->where('email_footers.id', 9)->first();
    ...
    /**
     * Show the form for creating a new DriverVerifyEmailSettings.
     *
     * @return Factory|View
     */
    ...
    {
        $languages = Language::where('status', 1)->get();
        return view('admin.DriverVerifyEmailSettings.create', ['languages' => $languages]);
    }

    /**
     * Store a newly created DriverVerifyEmailSettings in storage.
     *
     * @param Request $request
     * @return JsonResponse
     */
    public function FormOneCreate(Request $request)
    {
        if ($request->hasFile('emh_logo')) {
            ...
            $logo->move('./assets/DriverVerifyEmail/logo/', $logoName);
            $welcomeEmailLogo = 'assets/DriverVerifyEmail/logo/' . $logoName;
        }else{
            $welcome = EmailHeader::where('id',9)->first();
    ...
    /**
     * Display the specified DriverVerifyEmailSettings.
     *
     * @param int $id
     * @return Factory|View
     */
    public function show($id)
    {
        ...
        return view('admin.DriverVerifyEmailSettings.show', ['languages' => $languages, 'header' => $header, 'headerTrans' => $headerTrans,
            'bodyTrans' => $bodyTrans, 'footerTrans' => $footerTrans, 'socialLinks' => $socialLinks]);
    }

    /**
     * Show the form for editing the specified DriverVerifyEmailSettings.
     *
     * @param int $id
     * @return void
     */
