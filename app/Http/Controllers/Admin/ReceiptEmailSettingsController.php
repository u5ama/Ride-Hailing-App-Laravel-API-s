...
class ReceiptEmailSettingsController extends Controller
{
    /**
     * Display a listing of the ReceiptEmailSettings.
     *
    ...
    public function index(Request $request)
    {
        ...
        $header = EmailHeader::where('id', 4)->first();
        $headerTrans = EmailHeader::listsTranslations('name')->select('email_headers.id', 'email_headers.emh_logo', 'email_header_translations.emh_subject', 'email_header_translations.emh_title', 'email_header_translations.emh_description')->where('email_headers.id', 4)->first();
        $bodyTrans = EmailBody::listsTranslations('name')->select('email_bodies.id', 'email_body_translations.emb_title_text_bf_name', 'email_body_translations.emb_title_text_after_name', 'email_body_translations.emb_body_text_bf_button', 'email_body_translations.emb_body_text_after_button')->where('email_bodies.id', 4)->first();
        $footerTrans = EmailFooter::listsTranslations('name')->select('email_footers.id', 'email_footer_translations.emf_company_name', 'email_footer_translations.emf_company_address', 'email_footer_translations.emf_company_contacts')->where('email_footers.id', 4)->first();

        return view('admin.ReceiptEmailSettings.index', ['languages' => $languages, 'header' => $header, 'headerTrans' => $headerTrans, 'bodyTrans' => $bodyTrans, 'footerTrans' => $footerTrans]);
    }

    /**
     * Show the form for creating a new ReceiptEmailSettings.
     *
     * @return Factory|View
     */
    ...
    {
        $languages = Language::where('status', 1)->get();
        return view('admin.ReceiptEmailSettings.create', ['languages' => $languages]);
    }
    ...
    public function FormOneCreate(Request $request)
    {
        if ($request->hasFile('emh_logo')) {
            ...
            $logo->move('./assets/ReceiptEmail/logo/', $logoName);
            $welcomeEmailLogo = 'assets/ReceiptEmail/logo/' . $logoName;
        }else{
            $welcome = EmailHeader::where('id',4)->first();
    ...
    /**
     * Store a newly created ReceiptEmailSettings in storage.
     *
     * @param Request $request
     * @return JsonResponse
     */
    ...
    /**
     * Display the specified ReceiptEmailSettings.
     *
     * @param int $id
     * @return Factory|View
     */
    public function show($id)
    {
        ...
        return view('admin.ReceiptEmailSettings.show', ['languages' => $languages, 'header' => $header, 'headerTrans' => $headerTrans,
