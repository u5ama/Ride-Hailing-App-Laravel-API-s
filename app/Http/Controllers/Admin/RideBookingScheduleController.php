...
class RideBookingScheduleController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($start_date && $end_date) {
                ...
                $rideBookSchedule = RideBookingSchedule::query()->whereRaw("date(ride_booking_schedules.rbs_created_at) >= '" . $start_date . "' AND date(ride_booking_schedules.rbs_created_at) <= '" . $end_date . "'")->with('driver', 'passenger')->get();
            } else {
                $currentDate = date('Y-m-d');
                $rideBookSchedule = RideBookingSchedule::whereDate("ride_booking_schedules.rbs_created_at", $currentDate)->with('driver', 'passenger')->get();
            }
            ...
            return Datatables::of($rideBookSchedule)
                ...
                ->addColumn('rbs_driving_start_time', function ($rideBookSchedule) {
                    return Utility:: convertTimeToUSERzone($rideBookSchedule->rbs_driving_start_time,Utility::getUserTimeZone(auth()->guard('admin')->user()->time_zone_id));
                   
                })
                ->addColumn('rbs_driving_end_time', function ($rideBookSchedule) {
                    return Utility:: convertTimeToUSERzone($rideBookSchedule->rbs_driving_end_time,Utility::getUserTimeZone(auth()->guard('admin')->user()->time_zone_id));
                   
                })
                ->addColumn('rbs_driving_wait_start_time', function ($rideBookSchedule) {
                    return Utility:: convertTimeToUSERzone($rideBookSchedule->rbs_driving_wait_start_time,Utility::getUserTimeZone(auth()->guard('admin')->user()->time_zone_id));
                   
                })
                ->addColumn('rbs_driving_wait_end_time', function ($rideBookSchedule) {
                    return Utility:: convertTimeToUSERzone($rideBookSchedule->rbs_driving_wait_end_time,Utility::getUserTimeZone(auth()->guard('admin')->user()->time_zone_id));
        ...
        return view('admin.rideBookingSchedule.index');
    }
    ...
    {
        return view('admin.rideBookingSchedule.viewMapModal');
    }
