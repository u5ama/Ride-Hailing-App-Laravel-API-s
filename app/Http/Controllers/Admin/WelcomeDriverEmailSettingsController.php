...
class WelcomeDriverEmailSettingsController extends Controller
{
    /**
     * Display a listing of the WelcomeDriverEmailSettings.
     *
    ...
    public function index(Request $request)
    {
        ...
        $headerTrans = EmailHeader::listsTranslations('name')->select('email_headers.id', 'email_headers.emh_logo', 'email_header_translations.emh_subject', 'email_header_translations.emh_title', 'email_header_translations.emh_description')->where('email_headers.id', 7)->first();
        $bodyTrans = EmailBody::listsTranslations('name')->select('email_bodies.id', 'email_body_translations.emb_title_text_bf_name', 'email_body_translations.emb_title_text_after_name', 'email_body_translations.emb_body_text_bf_button', 'email_body_translations.emb_body_text_after_button')->where('email_bodies.id', 7)->first();
        $footerTrans = EmailFooter::listsTranslations('name')->select('email_footers.id', 'email_footer_translations.emf_company_name', 'email_footer_translations.emf_company_address', 'email_footer_translations.emf_company_contacts')->where('email_footers.id', 7)->first();
        return view('admin.welcomeDriverEmailSettings.index', ['languages' => $languages, 'header' => $header, 'headerTrans' => $headerTrans, 'bodyTrans' => $bodyTrans, 'footerTrans' => $footerTrans]);
    }

    /**
     * Show the form for creating a new WelcomeDriverEmailSettings.
     *
     * @return Factory|View
     */
    ...
    {
        $languages = Language::where('status', 1)->get();
        return view('admin.welcomeDriverEmailSettings.create', ['languages' => $languages]);
    }

    /**
     * Store a newly created WelcomeDriverEmailSettings in storage.
     *
     * @param Request $request
     * @return JsonResponse
     */
    public function FormOneCreate(Request $request)
    {
        if ($request->hasFile('emh_logo')) {
            ...
            $logo->move('./assets/welcomeDriverEmail/logo/', $logoName);
            $welcomeEmailLogo = 'assets/welcomeDriverEmail/logo/' . $logoName;
        }else{
            $welcome = EmailHeader::where('id',1)->first();
    ...
    public function FormTwoCreate(Request $request)
    {
        ...
        foreach ($languages as $language) {
            EmailBodyTranslation::updateOrCreate([
                ...
                'emb_body_text_bf_button' => $request->input($language->language_code . '_emb_body_text_bf_button'),
    ...
    /**
     * Display the specified WelcomeDriverEmailSettings.
     *
     * @param int $id
     * @return Factory|View
     */
    public function show($id)
    {
        ...
        return view('admin.welcomeDriverEmailSettings.show', ['languages' => $languages, 'header' => $header, 'headerTrans' => $headerTrans,
            'bodyTrans' => $bodyTrans, 'footerTrans' => $footerTrans, 'socialLinks' => $socialLinks]);
    }

    /**
     * Show the form for editing the specified WelcomeDriverEmailSettings.
     *
     * @param int $id
     * @return void
     */
