...
class InProgressScheduleRidesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($start_date && $end_date) {
                ...
                $rideBookSchedule = RideBookingSchedule::leftJoin('drivers', 'ride_booking_schedules.rbs_driver_id', '=', 'drivers.id')->where(['drivers.du_com_id'=>$company_id])->whereRaw("date(ride_booking_schedules.rbs_created_at) >= '" . $start_date . "' AND date(ride_booking_schedules.rbs_created_at) <= '" . $end_date . "'")->where('ride_booking_schedules.rbs_transport_type', $category)->whereNotIn('ride_booking_schedules.rbs_ride_status', ['Cancelled','Rejected', 'Completed'])->with('driver', 'passenger')->get();
            }elseif ($category) {
                $currentDate = date('Y-m-d');
                $rideBookSchedule = RideBookingSchedule::leftJoin('drivers', 'ride_booking_schedules.rbs_driver_id', '=', 'drivers.id')->where(['drivers.du_com_id'=>$company_id])->with('passenger', 'driver')->whereDate("ride_booking_schedules.rbs_created_at", $currentDate)->where('ride_booking_schedules.rbs_transport_type', $category)->whereNotIn('ride_booking_schedules.rbs_ride_status', ['Cancelled','Rejected', 'Completed'])->orderBy('ride_booking_schedules.rbs_created_at', 'DESC')->get();
            } else {
                $currentDate = date('Y-m-d');
                $rideBookSchedule = RideBookingSchedule::leftJoin('drivers', 'ride_booking_schedules.rbs_driver_id', '=', 'drivers.id')->where(['drivers.du_com_id'=>$company_id])->whereDate("ride_booking_schedules.rbs_created_at", $currentDate)->whereNotIn('ride_booking_schedules.rbs_ride_status', ['Cancelled','Rejected', 'Completed'])->with('driver', 'passenger')->get();
            }
        ...
        return view('company.inProgressScheduleRides.index', compact('categories'));
    }
    ...
    {
        return view('company.inProgressScheduleRides.create');
    }
