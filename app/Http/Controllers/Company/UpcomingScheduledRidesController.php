    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($start_date && $end_date) {
                ...
                $rideBookSchedule = UpcomingScheduleRides::leftJoin('drivers', 'upcoming_schedule_rides.usr_driver_id', '=', 'drivers.id')->where(['drivers.du_com_id'=>$company_id])->whereRaw("date(upcoming_schedule_rides.usr_created_at) >= '" . $start_date . "' AND date(upcoming_schedule_rides.usr_created_at) <= '" . $end_date . "'")->with('driver', 'passenger')->get();
            }elseif ($category) {
                $currentDate = date('Y-m-d');
                $rideBookSchedule = UpcomingScheduleRides::leftJoin('drivers', 'upcoming_schedule_rides.usr_driver_id', '=', 'drivers.id')->where(['drivers.du_com_id'=>$company_id])->whereDate("upcoming_schedule_rides.usr_created_at", $currentDate)->where('usr_transport_type', $category)->orderBy('upcoming_schedule_rides.usr_created_at', 'DESC')
                ->with('passenger', 'driver')->get();
            } else {
                $currentDate = date('Y-m-d');
                $rideBookSchedule = UpcomingScheduleRides::leftJoin('drivers', 'upcoming_schedule_rides.usr_driver_id', '=', 'drivers.id')->where(['drivers.du_com_id'=>$company_id])->whereDate("upcoming_schedule_rides.usr_created_at", $currentDate)->with('driver', 'passenger')->get();
            }
...
//                })
        ...
        return view('company.upcomingScheduleRides.index', compact('categories'));
    }
    ...
    {
        return view('company.upcomingScheduleRides.create');
    }
